...
APP_URL=$(now --npm --public -n $NOW_PROJECT -e PRIVATE_KEY=@fusion-benchmarks-private-key -e APP_ID=$APP_ID -e WEBHOOK_SECRET=@fusion-benchmarks-webhook-secret -e NODE_ENV="production" -e CHART_URL=$CHART_URL -e LOG_LEVEL=debug -T $NOW_TEAM -t $NOW_TOKEN)
